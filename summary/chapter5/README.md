...
마지막 ID는 Go 에서만 해당 json을 읽을 것 같으면 그냥 int64로 나둬도 된다. 하지만 웹 어플리케이션을 작성하는 경우 등 자바스크립트가 json을 읽게 되면 문제가 발생한다.
...
* 명명된 자료형은 marshal, unmarshal시 원하는 값을 받을 수 있도록 사용자가 직접 정의할 수 있다.
...
* interface{} 는 JSON으로 표현된 모든 데이터를 역직렬화할 수 있다.
